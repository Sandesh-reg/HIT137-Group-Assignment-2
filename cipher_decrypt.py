...
def _transform_text(text: str, shift1: int, shift2: int, direction: int) -> str:
    return "".join(
        _transform_char(c, shift1, shift2, direction)
        for c in text
    )


# reads a text file, encrypts its contents, and writes the result
def encrypt_file(
    shift1: int,
    shift2: int,
    input_path: str,
...
# reads an encrypted file, decrypts its contents, and writes the result
def decrypt_file(
    shift1: int,
    shift2: int,
    input_path: str,
