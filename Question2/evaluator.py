...
class _Parser:
    ...
    # Handles multiplication, division, modulo and implied multiplication.
    def parse_term(self):
    ...
    # Handles negative numbers and checks for unsupported unary plus.
    ...
    # Handles powers and makes them right associative.
    ...
    # Reads either a number or an expression inside brackets.
...
def parse(tokens):
    """Parse the full list of tokens and return the expression tree."""
...
# Converts the number into the format used in the expression tree.
...
# Turns the expression tree into a readable string.
...
# Calculates the final value from the expression tree.
...
# Formats the answer so whole numbers don't show unnecessary decimal places.
...
def evaluate_expression(expr: str) -> dict:
    """Evaluate one expression and return the input, tree, tokens and result."""
...
def evaluate_file(input_path: str) -> list:
    """Read the expressions from a file, evaluate them and create output.txt."""
    with open(input_path, "r", encoding="utf-8") as f:
        lines = [line.rstrip("\n").rstrip("\r") for line in f]

    # Skip blank lines so they aren't treated as expressions.
